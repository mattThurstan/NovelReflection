...
/*
 * (a) Novel Reflection device.
 * 
 * Forked from Simple Arduino Sun Tracking / Heliostat Control Program by Gabriel Miller ( http://www.cerebralmeltdown.com )                                                                       
 */
...
float latitude = 39.19;           // PUT YOUR LATITUDE, LONGITUDE, AND TIME ZONE HERE
float longitude = -78.16;
float timezone = -5;


int useNorthAsZero = 0;           // If you live in the northern hemisphere, put 0 here. If you live in the southern hemisphere put 1.
  
int potentiometerIsConnected = 1; // Put a 0 here if you have not connected the target changer potentiometer to the circuit. Put a 1 here if it is connected. 
 ...
 * Tells the program how many degrees to move off of limit switches after they have been triggered. Choose a positive number.
 */
float moveAwayFromLimit = 5;


/* TIME MACHINE(S) RESET 
 * Put the hour you want your solar machines to reset on their limit switches here. 
 * Remember that daylight saving time is not used, so the hour it resets won't necessarily match the clock in your home.
 */
int hourReset = 21;


float machineAltParkAngle=65;     // Machine wind protection Altitude parking angle.
float machineAzParkAngle=0;       // Machine wind protection Azimuth parking angle.

/* STEPPER MOTOR SETUP 
 * The number of steps required for your stepper to make one revolution. 
 * ( Don't forget to take into account the settings on your driver board. i.e. Microstepping, half stepping etc. )
 */
float steps = 800;
//float steps = 1600;

float altSpeed=40;                // Set the travel speed for your Altitude stepper motors here ( roughly in rev/min ).
float azSpeed=40;                 // Set the travel speed for your Azimuth stepper motors here ( roughly in rev/min ).
...
float altManualSpeed=40;          // Speed of Altitude stepper motors when using manual control through joystick.
float azManualSpeed=40;           // Speed of Azimuth stepper motors when using manual control through joystick.


float altAccel = 1;               // Number of seconds (roughly) to accelerate Altitude movement to full speed.
float azAccel = 1;                // Number of seconds (roughly) to accelerate Azimuth movement to full speed.

long altitudeMax = 1000000;       // Maximum number of steps the machine(s) will take on the Altitude when resetting.
long azimuthMax  = 1000000;       // Maximum number of steps the machine(s) will take on the Azimuth when resetting.
...
{  
  ...
  if ((now>=updateTime)||(iterationsAfterReset == 0)) 
  {
    ...
    if(useNorthAsZero==1) {
      if (SunsAzimuth<0) { SunsAzimuth=(SunsAzimuth+180)*-1; }
      if (SunsAzimuth>0) { SunsAzimuth=(SunsAzimuth-180)*-1; }
  ...
  for (int i=0; i <= numberOfMachines - 1; i++) 
  {
    ...
    moveMachine(
      PrevAltTargetsArray[machineNumber], 
      PrevAzTargetsArray[machineNumber],
      MachineTargetAlt[machineNumber], 
      MachineTargetAz[machineNumber],
