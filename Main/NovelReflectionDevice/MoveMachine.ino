...
float minAz, float minAlt, float maxAz, float maxAlt) 
{
  ...
  if ((iterationsAfterReset==0) || (FirstIterationAfterArduinoReset==0)) {//Machine Resets Position
    if ((digitalRead(WindProtectionSwitch)!= HIGH)) {
      ...
      resetPositionOfMachine(altMotorDirection, altLimitAngle, altGearRatio, altb, altc, altAcuteObtuse, altAngleAtZero, azMotorDirection, azLimitAngle, azGearRatio, azb, azc, azAcuteObtuse, azAngleAtZero);
    }
  }
  ...
  // Waits until the sun is above the horizon before moving normally.
  // This code will also run if either manual mode or wind protection mode is activated.
  if (SunsAltitude>=0 || digitalRead(WindProtectionSwitch)==HIGH || digitalRead(manualModeOnOffPin)==HIGH) 
  {
    
    // 1 - Machine Acts as Dual Axis Sun Tracker
    if (sunTrackerOrHelio == 1) { 
       MachinesNewAltitude = SunsAltitude;
       MachinesNewAzimuth = SunsAzimuth;
    }
  
    // 2 - Machine Acts as Dual Axis Sun Tracker
    if (sunTrackerOrHelio == 2) { 
    ...
    if ((MachinesNewAzimuth > minAz) && (MachinesNewAltitude > minAlt) && (MachinesNewAzimuth < maxAz) && (MachinesNewAltitude < maxAlt)) 
    {
      ...
      //This code does not run directly after machine resets.
      if (iterationsAfterReset > 0) {
        ...
        if (abs(altsteps)>0 || abs(azsteps)>0) {
          if(joystickModeOnOff!=1) { MachineOn(machineNumber); }
